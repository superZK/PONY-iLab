...
export default class TaskVerifyDialog extends Component {
  state = {
    preparedItemKeys: [],
    userAccount: '',
   };
  ...
  render(){
    ...
    const infoFormSetting = DataVerifySetting.form.taskInfoForm;
    const sampleVerifySetting = DataVerifySetting.table.sampleVerify;

    const handleTableSelect = (rows, rowKeys) => {
      let taskIds = rows.map( item => item.taskId);
      this.setState({
        preparedItemKeys: taskIds || [],
      });
    }
