...
export default class ResultVerifyDialog extends Component {
  state = {
    preparedItemKeys: [],
    userAccount: '',
   };
  ...
  render(){
    ...
    const resultVerifySetting = DataVerifySetting.table.resultVerify;

    const handleTableSelect = (rows, rowKeys) => {
      let resultIds = rows.map( item => item.resultId);
      this.setState({
        preparedItemKeys: resultIds || [],
      });
    }
