...
import { uniqueObj, unique, replaceNodeById, removeNodeById, analysisDataIndex, findParentNodeById, findParentById } from '../../util/treeUtils';
 ...
 * renderType:渲染类型 ->'文字提示'(用于展示长内容)、'图标'、'状态点'、'可编辑'
 * render:{
 *    ***********文字提示***********
...
export default class AdvancedTable extends Component {
  ...
  componentWillReceiveProps(nextProps){
    // 原始数据变化时变更state：dataSource，配合可编辑使用
 ...
 */
  ...
  switchRenderType(columnItem, text, record){
    ...
    switch(columnItem.renderType){
      ...
      case '状态点':
        return (columnItem.render.processRule('已完成'));
      case '可编辑':
        return (
          <EditableTableCell
            value={value || ''}
        ...
        <Row gutter={16} style={{textAlign: 'right'}}>
          {config.map( (item) => {
            ...
            if(typeof(record[item.key]) === 'boolean'){
              ...
              return (
              <Col span={8} >
                <label style={lableStyle}>{item.label}:</label><TooltipSpan mouseEnterDelay={0.5} longNum={20} data={analysisDataIndex(record, item.key)}/>
              </Col>
              );
            }
          } )}
